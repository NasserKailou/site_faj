...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'ajouter' || $action === 'modifier') {
            ...
            $type   = sanitize($_POST['type'] ?? 'institutionnel');
            ...
            if (empty($nom)) { $err = 'Le nom est obligatoire.'; }
                ...
                if ($action === 'ajouter') {
                    $pdo->prepare("INSERT INTO partenaires (nom,type,logo,site_web,ordre,actif) VALUES (?,?,?,?,?,?)")
                        ->execute([$nom,$type,$logo,$site,$ordre,$actif]);
                    $msg = 'Partenaire ajouté.';
                } else {
                    $id = (int)($_POST['id'] ?? 0);
                    $pdo->prepare("UPDATE partenaires SET nom=?,type=?,logo=?,site_web=?,ordre=?,actif=? WHERE id=?")
                        ->execute([$nom,$type,$logo,$site,$ordre,$actif,$id]);
...
<div class="page-header">
    <div>
        <h1 class="page-title"><i class="fas fa-handshake"></i> Partenaires</h1>
        <p class="page-subtitle"><?= count($partenaires) ?> partenaire(s)</p>
    </div>
...
<table class="table">
<thead><tr><th>#</th><th>Logo</th><th>Nom</th><th>Type</th><th>Site Web</th><th>Ordre</th><th>Statut</th><th>Actions</th></tr></thead>
<tbody>
...
<tr>
    ...
    <td>
        <?php if ($p['logo']): ?>
        <img src="<?= UPLOADS_URL ?>/partenaires/<?= $p['logo'] ?>" style="height:40px;width:60px;object-fit:contain;border-radius:4px;">
        <?php else: ?><span style="color:var(--admin-gray);">-</span><?php endif; ?>
    </td>
    <td><strong><?= sanitize($p['nom']) ?></strong></td>
    <td><span class="badge orange"><?= $p['type'] ?></span></td>
    <td><?= $p['site_web'] ? '<a href="'.sanitize($p['site_web']).'" target="_blank" style="color:var(--admin-primary);">Lien</a>' : '-' ?></td>
    <td><?= $p['ordre'] ?></td>
    <td><?= $p['actif'] ? '<span class="status-badge success">Actif</span>' : '<span class="status-badge danger">Inactif</span>' ?></td>
    <td>
        <button onclick='editPart(<?= json_encode($p) ?>)' class="btn btn-sm btn-outline btn-icon"><i class="fas fa-edit"></i></button>
...
<div id="modalPart" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:1000;align-items:center;justify-content:center;">
    <div style="background:white;border-radius:var(--admin-radius);padding:36px;width:520px;max-width:95vw;max-height:90vh;overflow-y:auto;">
        <h3 id="partModalTitle" style="margin-bottom:24px;color:var(--admin-primary);font-size:18px;"><i class="fas fa-handshake"></i> Ajouter un partenaire</h3>
        <form method="POST" enctype="multipart/form-data">
            ...
            <div style="margin-bottom:16px;"><label style="font-size:12px;font-weight:600;display:block;margin-bottom:6px;">Logo (JPG, PNG, SVG)</label><input type="file" name="logo" class="form-control" accept="image/*"><div id="partLogoPreview" style="margin-top:8px;"></div></div>
            ...
            <div style="display:flex;gap:12px;justify-content:flex-end;">
                <button type="button" onclick="document.getElementById('modalPart').style.display='none'" class="btn btn-outline">Annuler</button>
...
function editPart(p) {
    ...
    document.getElementById('pType').value = p.type || 'institutionnel';
    ...
    if (p.logo) {
        document.getElementById('partLogoPreview').innerHTML = '<img src="<?= UPLOADS_URL ?>/partenaires/' + p.logo + '" style="height:40px;border-radius:4px;">';
    }
